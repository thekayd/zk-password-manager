...
export default function BiometricRegister() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // then saves the credentials to Supabase
      const saveCredentials = await saveCredentialId(credentialId, email);
      const saveAuthnKey = await saveWebAuthnPublicKey(credential, email);

      if (!saveCredentials) throw new Error('Failed to save biometric ID');
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-50">
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-lg shadow-md">
        <div>
          ...
          <p className="mt-2 text-center text-gray-600">Set up biometric authentication for faster login (Optional)</p>
        </div>
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-600 px-4 py-3 rounded relative" role="alert">
            <span className="block sm:inline">Biometric registration successful!</span>
          </div>
        )}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleRegister}>
          <div>
            ...
            <input
              ...
              disabled
            />
            <p className="mt-2 text-sm text-gray-500">Email from your registration</p>
          </div>
